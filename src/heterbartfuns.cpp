...
{
  //printf("*****MODIFIED IT heterbart::heterdrawnodemu in src!!!!!! \n");
   //LRF:this is what is called by draw in 
   double muhat = M/b;
   double a = 1.0/(tau*tau);
   //LRF: muhat should be the mean residual vector for obs that fall in that node
...
void heterallsuff(tree& x, xinfo& xi, dinfo& di, tree::npv& bnv, std::vector<double>& bv, std::vector<double>& Mv,double *sigma)
{
   ...
   for(size_t i=0;i<di.n;i++) {
      ...
      bv[ni] += w; //CHECK WITH DRAWNODEMU
      Mv[ni] += w*di.y[i]; //LRF: NEED TO MODIFY - USED IN DRAWNODEMU
   }
}
//--------------------------------------------------
...
void heterdrmu(tree& t, xinfo& xi, dinfo& di, pinfo& pi, double *sigma, rn& gen)
{
   ...
   // loop over the size of the tree - bnv.size()?
