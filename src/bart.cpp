...
//--------------------------------------------------
void bart::draw(double sigma, rn& gen, double *u_asym)
{
  //not called by cwbart
  //printf("*****MODIFIED IT bart::draw in src!!!!!! \n");
   for(size_t j=0;j<m;j++) {
      fit(t[j],xi,p,n,x,ftemp);
      for(size_t k=0;k<n;k++) {
         allfit[k] = allfit[k]-ftemp[k] + u_asym[k];
         r[k] = y[k]-allfit[k];
      }
